...
- **Operators** can confirm a CSR + profile combination will be accepted before
  ...
  `require_approval` profile ([approvals](approvals.md)) and you want to know the
...
`certificate_policy`, `smime` ([S/MIME](smime.md)), `keycheck`
([weak-key & compromised-key gate](key-checks.md)), `upn`
([smartcard-logon / PKINIT](smartcard-logon.md)), `qcstatements`
([eIDAS qualified certificates](qualified-certificates.md)), `validity`, plus
...
- **`attestation`** — the profile's enrollment key-attestation posture
  ([enrollment attestation](enrollment.md)). Attestation is enforced on the
...
- [Weak-key & compromised-key gate](key-checks.md) — the keycheck gate the
  preview reports.
- [Certificate authority](certificate-authority.md) — the real issuance path.
- [Four-eyes / maker-checker approvals](approvals.md) — the `park` decision.
- [Operator web console](web-console.md) — the Issue page's preview button.
