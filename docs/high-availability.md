...
[OCSP pre-signing & publishing](ocsp-presign-publish.md)); the leader keeps
...
- The chaos suite (`scripts/chaos-test.sh`) runs
  ...
  chain intact throughout. See [resilience](resilience.md).

## See also

- [Persistence backends](persistence.md) — pointing replicas at a shared
  PostgreSQL and sizing its pool.
- [Expiry monitoring](expiry-monitoring.md), [CA rotation](ca-rotation.md),
  [OCSP presign & publishing](ocsp-presign-publish.md),
  [audit SIEM export](audit-siem-export.md), and
  [RBAC & audit](rbac-and-audit.md) (anchoring) — the gated jobs.
