...
| **Key generation ceremony / witnessed, scripted, logged** (BR §6.1.1.1; WebTrust key-lifecycle) | M-of-N quorum ceremony with hash-committed operator confirmations, audit-anchored. `secsy-ca ceremony`, [key ceremony](key-ceremony.md) | ⚙️ / 👤 | Software provides the quorum + transcript; operator supplies witnesses, script sign-off, and video/records per their policy. |
...
| **Systems development & maintenance** | SBOM + cosign signatures + SLSA provenance + govulncheck; reproducible-ish tagged releases. root `Makefile`, `release.yaml`; [supply-chain](supply-chain.md) | ✅ / 👤 | Operator's own change-control and deployment pipeline. |
...
- [Architecture Decision Records](adr/README.md) · [Security review](security-review.md) ·
  [Pre-issuance linting](certlint.md) · [CAA](caa.md) ·
  [Certificate Transparency](certificate-transparency.md) · [FIPS mode](fips.md) ·
  [RBAC & audit](rbac-and-audit.md) · [Key ceremony & DR](key-ceremony.md).
