...
revocation increment the same Prometheus certificate counters as REST.
...
# Individual operations: issue | renew | revoke | get | status | list |
# crl-metadata | ocsp-metadata. Authenticate with -token (Bearer), -basic, or
# -client-cert/-client-key (mTLS). Use -plaintext for a local cleartext server.
...
[Kubernetes deployment](kubernetes.md#grpc-api). Set `config.grpc.enabled=true`;
