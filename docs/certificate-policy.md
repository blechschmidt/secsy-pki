...
offload (`internal/publish`, [docs](ocsp-presign-publish.md)). Publication uses
...
runbook](incident-response.md)). **[OPERATOR: publish how a third party files a
...
- **Suspension (certificateHold) and release.** Reversible hold
  ...
  docs](certificate-authority.md)). **[OPERATOR: the BRs prohibit suspension for
...
- **Separation of duties / dual control.** The **four-eyes / maker-checker gate**
  ...
  0006](adr/0006-four-eyes-approval-gate.md), [approvals docs](approvals.md)).
...
- **Backups.** A leader-elected job produces a DR artifact (logical DB dump +
  ...
  (`internal/backup`, [backup docs](backup.md)). **No private CA key is ever in
...
- **DR drills.** `scripts/dr-drill.sh` and `scripts/dr-drill-full.sh` exercise
  ...
  continuity ([key ceremony & DR](key-ceremony.md)).
...
- **Ceremony.** Root/intermediate generation can be run as an **M-of-N quorum
  ...
  (`secsy-ca ceremony`, [key ceremony docs](key-ceremony.md)).
...
- **Static analysis, race detection, fuzzing, and govulncheck** gate the build
  ...
  [supply-chain docs](supply-chain.md)); releases carry SBOMs, cosign
...
issuance (`internal/fips`, [FIPS docs](fips.md)).
...
- [Security review & hardening](security-review.md) · [Key ceremony & DR](key-ceremony.md) ·
  [RBAC & audit](rbac-and-audit.md) · [Certificate authority](certificate-authority.md) ·
  [CAA](caa.md) · [Certificate Transparency](certificate-transparency.md) ·
  [Pre-issuance linting](certlint.md) · [FIPS mode](fips.md).
