...
- **MS-XCEP** ([\[MS-XCEP\]](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-xcep/08ec4475-32c2-457d-8c27-5a176660a210)) —
  ...
  client (mapped from secsy-pki [issuance profiles](certificate-authority.md)),
...
- **MS-WSTEP** ([\[MS-WSTEP\]](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-wstep/4766a85d-0d18-4fa1-a51f-e5cb98b752ea)) —
  ...
  **HSM-backed** [`ca.Manager`](certificate-authority.md) and returns it as a
  certs-only PKCS#7 in a `RequestSecurityTokenResponse` (RSTR).

Like the other enrollment protocols ([SCEP/EST/CMP](enrollment.md), [ACME](acme.md)),
...
[tamper-evident audit log](rbac-and-audit.md). The services mount **outside** the
...
|---|---|---|
| **Native API token** ([Task 86](authentication.md)) | `Authorization: Token secsy_pat_…` (or `Bearer secsy_pat_…`) | must hold the **`cert:issue`** capability in the CA's tenant |
| **Operator mutual-TLS** | a client certificate verified against the operator client-CA pool ([`auth.mtls`](authentication.md)) | resolved to a principal; must hold **`cert:issue`** in the CA's tenant |
...
tokens are always accepted, and when [`auth.mtls`](authentication.md) is enabled
...
[rate-limit + HSM-concurrency guard](rate-limiting.md) exactly like the other
...
[lint](certlint.md), [CAA](caa.md), [name constraints](name-constraints.md),
[UPN SANs](smartcard-logon.md) — so a Windows machine template and a secsy-pki
...
Related: [SCEP/EST/CMP enrollment](enrollment.md) ·
[Smartcard-logon & PKINIT](smartcard-logon.md) ·
[Native API tokens](authentication.md) · [Operator authentication](authentication.md) ·
[Certificate profiles](certificate-authority.md) · [Rate limiting](rate-limiting.md)
