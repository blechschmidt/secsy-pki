...
A high-volume CA — short-lived [ACME STAR](acme.md) certificates, automated
...
- **leadership** is the Task 68 [advisory-lock elector](high-availability.md) — a
  singleton, so replicas never race each other's archive/prune transactions;
- **freshness** surfaces through the [tamper-evident audit log](audit-siem-export.md)
...
2. it is not on hold (`status != 'held'` — a hold is reversible, so the row is
   not terminal); **and**
3. no open (pending/approved) four-eyes [approval](approvals.md) pins its serial.
