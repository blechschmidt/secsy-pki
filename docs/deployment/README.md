...
replica, how the server obtains its own serving certificate, and how to take the
listeners off the network altogether.

| Guide | Covers |
|-------|--------|
| [**The container image**](container.md) | The published `ghcr.io/blechschmidt/secsy-pki` image: which of the `latest`/`X.Y.Z`/`X.Y`/`edge`/branch/`sha-…` tags to pull and which are mutable, the two architectures, the six commands inside it and the non-root account they run as, why SoftHSM ships in it and what to mount over it in production, the `cosign`/`slsa-verifier` checks to run before trusting a digest, `scripts/verify-published-image.sh` for checking it actually works, and the continuous three-job build that produces it — plus running it: a first run from `docker pull` to an issued certificate, the two config paths (and why the CLIs do not share the server's), which volumes hold state, the `SECSY_*` environment overrides, the CLI tools via `exec` or a one-shot container, the `deploy/compose/` stack (SQLite or PostgreSQL), `/healthz`+`/readyz`+`/metrics` and a healthcheck for an image with no `curl`, upgrades, and a troubleshooting list of the errors it actually prints |
...
| [**Unix-domain-socket listeners**](unix-socket.md) | Serving the HTTP/console and gRPC surfaces on a socket instead of a TCP port (`server.unix_socket` / `grpc.unix_socket`): no port bound at all, socket ownership/mode as the access boundary (race-free `chmod`, safe stale-socket reclaim), why cleartext needs no `SECSY_ALLOW_INSECURE_HTTP` there, `SO_PEERCRED` peer-uid attribution in the access log/audit chain/rate-limit buckets, `unix://` clients, and the `doctor listener.unix_socket` check |
