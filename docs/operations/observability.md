...
the request log via `trace_id`/`span_id`), see [docs/operations/tracing.md](tracing.md). It
...
jobs (see [multi-replica coordination](../deployment/high-availability.md)). It is
...
| `secsy_leader_is_leader` | gauge | — | 1 on the replica leading the singleton background jobs, 0 on followers (see [high availability](../deployment/high-availability.md)) |
...
- [RBAC, audit logging & config](../security/rbac-and-audit.md) — the event log the request
  IDs correlate to.
- [Certificate authority](../ca/overview.md) — the issuance/OCSP/CRL
