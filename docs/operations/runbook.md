...
- **Root CA:** there is no online recovery. Distribute a revocation/notice out
  of band, stand up a new root via a fresh [key ceremony](../hsm/key-ceremony.md), and
...
|----------|---------|-------|
| ACME (RFC 8555) | `/acme/directory`, `/acme/new-nonce`, `/acme/new-account`, `/acme/new-order`, `/acme/order/{id}`, `/acme/order/{id}/finalize`, `/acme/authz/{id}`, `/acme/chall/{id}`, `/acme/cert/{id}`, `/acme/revoke-cert`, `/acme/key-change` | [acme.md](../protocols/acme.md) |
| ACME ARI | `/acme/renewal-info/{certid}` | [acme.md](../protocols/acme.md) |
| SCEP (RFC 8894) | `/scep` (and `/scep/pkiclient.exe`), `?operation=getcacaps\|getcacert\|pkioperation` | [enrollment.md](../protocols/scep-est.md) |
| EST (RFC 7030) | `/.well-known/est/cacerts`, `/simpleenroll`, `/simplereenroll`, `/serverkeygen` | [enrollment.md](../protocols/scep-est.md) |
| TSA (RFC 3161) | `/tsa` | [timestamping.md](../signing/timestamping.md) |
...
([rate-limiting.md](../security/rate-limiting.md)). Both live under `rate_limit:` in
`config.yaml`.
...
[the load-test suite](../development/benchmarks.md)); a YubiHSM sustains far fewer concurrent
...
([certificate-transparency.md](../issuance/certificate-transparency.md)). Behavior on a CT
...
The [inclusion monitor](../issuance/certificate-transparency.md#inclusion-proof-monitoring-post-issuance)
...
([high availability](../deployment/high-availability.md)), not the logs.
...
[four-eyes approval](../security/approvals.md): the *maker* submits (the operation parks with
...
Native `secsy_pat_` tokens ([authentication](../security/authentication.md#4-native-scoped-api-tokens-service-accounts))
...
procedure and rationale live in [key-ceremony.md](../hsm/key-ceremony.md); rehearse it
with the drill.
...
back to a fresh [key ceremony](../hsm/key-ceremony.md) and re-issue the hierarchy, as
...
PostgreSQL persistence backend ([persistence.md](../deployment/persistence.md)), the database
...
deployments — see [high availability](../deployment/high-availability.md)): each replica's
...
the exporter cursor; see [audit-siem-export.md](../security/audit-siem-export.md).
...
- A goroutine dump (`goroutine?debug=2`) shows how many requests are parked in the
  ...
  [benchmarks.md](../development/benchmarks.md)) or reduce concurrency upstream.
...
threshold is crossed. See [docs/signing/trusted-time.md](../signing/trusted-time.md) for protocol
detail.
...
enforcement, the `make` targets): [supply-chain.md](../development/supply-chain.md).
...
| `fips.mode` | (with `security.fips: true`) the process runs on the Go FIPS 140-3 module (see [FIPS mode](../security/fips.md)) | warn: policy enforced on a non-module binary (build with `make build-fips`) |
...
[serving-cert.md](../deployment/serving-cert.md).
...
| A client can't verify our cert | `secsy-ca validate-cert -ca <id> leaf.pem` (chain/validity/live revocation/constraints; [chain validation](../ca/chain-validation.md)) |
...
[security review](../security/security-review.md) for the hardening baseline, and the
