...
Backup and restore (the [key ceremony & DR](../hsm/key-ceremony.md) tooling) and the
...
HSM-backed [secret/envelope layer](../secrets/password-encryption.md), and writes it to a
...
- **leadership** is the Task 68 [advisory-lock elector](../deployment/high-availability.md);
...
[key ceremony & DR](../hsm/key-ceremony.md). The artifact restores independently of the
...
key provider (see [password encryption](../secrets/password-encryption.md) for
provisioning).
...
[monitor notification sinks](expiry-monitoring.md) the expiry monitor uses
...
[DR runbook](../hsm/key-ceremony.md). The `internal/backup` package exposes
