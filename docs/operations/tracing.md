...
- **`hsm.session.acquired`** — a PKCS#11 session was borrowed from the bounded
  pool ([Task 20](../development/benchmarks.md)); carries `hsm.session.wait_ms` and
...
- **`hsm.token.error`**, **`hsm.failover`**, **`hsm.failover.served`** — the
  multi-token high-availability path ([HSM HA](../hsm/high-availability.md)). When an operation
