...
| [SIEM export](audit-siem-export.md) | Log collectors (syslog/CEF/webhook) | Streams the **whole** audit log to fixed sinks with a durable cursor |
...
- **Leader-elected.** The worker runs as a singleton
  [leader-elected job](high-availability.md), so a multi-replica deployment
