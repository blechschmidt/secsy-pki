...
is signed by an **HSM-backed CA** through the same [`ca.Manager`](../ca/overview.md)
used by the REST API and CLI, and every order is written to the
[tamper-evident audit log](../security/rbac-and-audit.md).
...
An "ACME-enabled profile" is simply the certificate [profile](../ca/overview.md#3-certificate-profiles)
...
[enrollment.md §7](scep-est.md#7-acme-smime-certificates-rfc-8823-email-reply-00)
...
Every ACME operation appends an entry to the [hash-chained event log](../security/rbac-and-audit.md):
...
[metrics endpoint](../operations/observability.md). Issuance is metered per profile:
...
- *Normal:* the window begins `acme.renewal_window_days` before expiry (falling
  back to the [expiry monitor's](../operations/expiry-monitoring.md) `renew_before_days`, then
...
[expiry monitor](../operations/expiry-monitoring.md). Renewal never involves the client.
...
- **TLS.** Real ACME clients require the directory to be served over HTTPS.
  ...
  [fails closed without TLS](../security/security-review.md) unless explicitly overridden.)
...
- **Nonces** are single-use anti-replay tokens (RFC 8555 §6.5) and are correct
  ...
  > [Rate limiting](../security/rate-limiting.md) and [High availability](../deployment/high-availability.md).
...
- [Certificate authority](../ca/overview.md) — creating the issuing CA and profiles
- [RBAC, audit logging & config](../security/rbac-and-audit.md) — the event log and roles
