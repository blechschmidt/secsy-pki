...
- **Deterministic jitter, no renewal storms.** The exact renewal moment is
  ...
  — the client-side counterpart of the [expiry monitor](../operations/expiry-monitoring.md)'s
  storm prevention.
...
the same way as for [server-side monitoring](../operations/observability.md), e.g.
