...
top of the existing [EST enrollment](scep-est.md) and the
[hardware-attestation](../security/authentication.md) trust anchors. The LDevID is issued
...
1. **The IDevID is authentic** — its certificate chains to a configured trusted
   ...
   [attestation gate](../security/authentication.md) uses; the registrar reuses them and any
...
[`internal/cms`](scep-est.md) layer. Its `pinned-domain-cert` is the registrar
...
[audit log](../security/rbac-and-audit.md) under action **`cert.brski`**, actor
`brski-pledge:<serial>`:
...
[rate-limit + HSM concurrency guard](../security/rate-limiting.md) like the other signing
