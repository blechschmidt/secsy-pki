...
| [**Per-CA & per-key authorization (resource grants)**](resource-grants.md) | Authorizing users and groups on **one** individually-addressed CA or signing key, so a platform administrator can keep the root CA while a specific group administers a single subordinate: the `ca-admin`/`ca-manager`/`ca-issuer`/`ca-auditor` and `key-admin`/`key-signer`/`key-auditor` roles, `self` vs `subtree` scope, declarative `rbac.grants` config plus the `/api/grants` API, `secsy-ca grant` CLI and console **Access** page, effective-access review, and the `resource.grant`/`resource.revoke` audit trail |
