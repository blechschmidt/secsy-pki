...
(built in [Kubernetes deployment](kubernetes.md)) so that consumers can prove,
...
(see [Cloud KMS backend](cloud-kms.md)).
...
See the [operator runbook](RUNBOOK.md) for the incident procedure when
verification fails in the field, and [Kubernetes deployment](kubernetes.md) for
