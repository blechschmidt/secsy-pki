...
- **certlint** runs inside `buildLeaf` before signing. In `enforce` mode a lint
  ...
  [certlint](../security-review.md) and the `secsy-ca lint` CLI.
- **CAA** (RFC 8659) is checked in `buildLeaf` for DNS identifiers. The
  ...
  authorized. See [CAA record checking](../caa.md).
...
- **A DNS or CT outage can stop issuance.** That is the intended trade-off for
  `enforce`/fail-closed configuration. The [runbook](../RUNBOOK.md) documents
...
- **Note the deliberate exception:** Certificate Transparency
  ([Task 26](../certificate-transparency.md)) is configurable *per profile* as
  ...
  choice — see [ADR rationale in the runbook](../RUNBOOK.md#ct-log-outage).
