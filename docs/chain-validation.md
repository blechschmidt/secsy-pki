...
- resolves **live revocation** against the CA's own CRL/OCSP state (the same
  ...
  state ([suspend/hold & release](certificate-authority.md)).
...
- **`name_constraints`** — the leaf's names lie within every issuer's RFC 5280
  Name Constraints ([Name Constraints](name-constraints.md)); `skipped` when no
...
- [Certificate authority](certificate-authority.md) — issuance, revocation, and
  suspend/hold, the states this validator reports.
- [Name Constraints & Certificate Policies](name-constraints.md) — the
  constraint/policy dimensions.
- [Weak-key & compromised-key gate](key-checks.md) — the weak-key flags shown per
  chain certificate.
- [gRPC API](grpc-api.md) — the `ValidateChain` RPC.
- [Operator web console](web-console.md) — the Validate page.
