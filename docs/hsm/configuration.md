...
- `module_path` is the vendor's PKCS#11 shared object. Common paths:
  ...
  Debian installs modules under a multiarch directory
  ...
  builds Yubico's module itself and symlinks it to the
  architecture-independent path above, so the same config file is correct on
  `amd64` and `arm64`.
