...
[HSM / PKCS#11 configuration](configuration.md)) and the bounded session pool
(see [benchmarks](../development/benchmarks.md)); each token in the set is an ordinary pooled
...
[duplicate-label](configuration.md#key-labels-must-be-unique) hazard). The HA
