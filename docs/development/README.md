...
| [**Releasing**](releasing.md) | Turning a `v*` tag into a signed image, two architectures of release archives and a GitHub release: the `CHANGELOG.md` section that becomes the release body, the four-fact release guard (and its own test suite) that refuses a tag before anything is built, the job order that puts every cheap reversible check ahead of every expensive permanent one, why the release calls the CI suite and the container workflow rather than repeating them, reproducible archives built from the image's own Dockerfile against glibc 2.36, the tagless dry run, and what to do when a release fails part-published |
