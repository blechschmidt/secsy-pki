...
[HA provider](../hsm/high-availability.md), and pulls the primary token out mid-load. Every
...
> the locked update path. See [persistence](../deployment/persistence.md).
...
[middleware](../security/rate-limiting.md) with a one-token burst and asserts the second
...
[coordination](../deployment/high-availability.md) invariants:
